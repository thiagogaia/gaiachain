...
class Blockchain implements \Countable
{
  ...
  public function add(Transaction $transaction) {
    $this->blocks[] = new Block($transaction, $this->blocks[$this->count() - 1]);
  }
  ...
  private function areSpendsValid() {
    foreach ($this->computeBalance() as $pubKey => $amount) {
      if ($amount < 0) {
        return false;
      }
    }
  ...
  public function balancesAsString() {
    ...
    foreach($this->computeBalances() as $pubKey => $amount) {
      $data[] = sprintf("%s = %s gaiacoins", substr($pubKey, 72, 7), $amount);
    }
    return implode("\n", $data);
  }
